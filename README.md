...
```
  ...
  │   3. 정규식 PII 탐지                          │
  │   4. 원본 로그 마스킹                         │
  │   5. 유사 사례 조회   (Security KB)           │
  │   6. LLM 위험도 판정  Solar Pro 2 → Gemini    │
  │   7. 학습 대상 판단                           │
  └───────────────────────────────────────────────┘
          │                          │
          ▼                          ▼
...
위 그림의 1번(Raw Queue 적재)과 2번(룰 기반 선분류)은 이 파일에 없고, Slack 알림 노드는 꺼져 있습니다.
...
| CRM_APP (CRM 시스템) | 454 |
| MAIL_GATEWAY (메일 게이트웨이) | 154 |
| DB_QUERY_LOG (DB 쿼리 로그) | 136 |
...
대상 중 완료는 4,649건이고 29건은 완료 표시가 없습니다. 4,656건에는 대상이 아닌데 완료로 찍힌 7건이 섞여 있습니다. 둘 다 왜 생겼는지는 아직 확인하지 못했습니다.

<br/>

## 4. 26시간 돌린 결과

2025-11-18 14:36 부터 11-19 16:35 까지(UTC)의 기록입니다.
...
뒤쪽의 CARD 는 카드번호가 아닙니다. 13자리 주민번호가 카드 정규식(13~16자리)에도 같이 걸린 것입니다.
...
저장된 텍스트에 남은 치환 토큰은 `[IP]` `[PHONE]` `[EMAIL]` `[RRN]` 네 가지이고 `[CARD]` 는 한 건도 없습니다.
주민번호 136건은 `앞6자리-[RRN]` 모양이라 앞 6자리가 남아 있습니다. 내보낸 워크플로의 정규식대로라면 통째로 `[RRN]` 이 됩니다.
...
├── backend/
│   ├── server-sqlite.js                       1,071줄, 라우트 20개
│   ├── server.js                              936줄, PM2 가 띄우지 않는 다른 서버 파일
...
`logs` 테이블은 id 를 빼고 컬럼이 27개입니다. 운영하면서 필요한 게 생길 때마다 붙였고
...
- 유사 사례 조회는 같은 사고 유형의 최근 3건을 가져오는 방식입니다. 로그 내용은 비교하지 않습니다. 임베딩 비교가 다음 과제입니다
